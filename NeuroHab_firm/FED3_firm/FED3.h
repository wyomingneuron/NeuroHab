...
class FED3 {
    ...
    public:
        ...
        void ConditionedStimulus(int duration);
        ...
        void BNC(int DELAY_US, int loops);
        ...
        void logLeftPoke();
        void logRightPoke();
        void Feed(char color);
        void Feed(bool pixelsoff=true, char color='G');
        // void dispenseLeftWater();
        // void dispenseRightWater();
        ...
        int FED;
        int FR = 1;
        int FR_Count_Right = -1;  // if -1 it is not applicable.
        int FR_Count_Left = -1;
        int RPR_Right = -1;
        int RPR_Count_Right = -1;
        int RPR_Left = -1;
        int RPR_Count_Left = -1;
        ...
        // encoding
        const int rightPoke = 1;        // NeuroHab TTL Encoding
        const int leftPoke = 2;         // NeuroHab TTL Encoding
        const int dispensed = 3;        // NeuroHab TTL Encoding
        const int retrieved = 4;        // NeuroHab TTL Encoding
        const int rightWater = 5;       // NeuroHab TTL Encoding
        const int leftWater = 6;        // NeuroHab TTL Encoding
        const int condStim = 7;         // NeuroHab TTL Encoding
        const int stimTone = 8;         // NeuroHab TTL Encoding
        const int frontWater = 9;       // NeuroHab TTL Encoding
        const int waterDispensed = 10;  // NeuroHab TTL Encoding
        const int rightWPellet = 11;    // NeuroHab TTL Encoding
        const int leftWPellet = 12;     // NeuroHab TTL Encoding
        const int rightDurDisp = 13;    // NeuroHab TTL Encoding
        const int leftDurDisp = 14;     // NeuroHab TTL Encoding
        const int rightInTimeout = 15;  // NeuroHab TTL Encoding
        const int leftInTimeout = 16;   // NeuroHab TTL Encoding
        
        uint64_t t1 = 0;  // Useed for latency benchmarking.
        uint64_t t2 = 0;


        // TTL pulse length in microseconds. 500 matches the neurohab
        int TTLus = 500;
        const int PULSE_SEPAR_DELAY_MS = 60;
