...
void FED3::run() {
  ...
  // DateTime now = rtc.now();  // REMOVE now BLOCK FOR NEUROHAB
  // currentHour = now.hour(); //useful for timed feeding sessions  REMOVE FOR NH
  // currentMinute = now.minute(); //useful for timed feeding sessions  REMOVE FOR NH

  // Detect second change for syncing ms.

  unsigned long cur_millis = millis();
  if (!initialized_millis) {
    init_millis = cur_millis;
    initialized_millis = true;
  }

  // unixtime = now.unixtime();  // REMOVE FOR NEUROHAB

  // Serial.print("time: ");  // REMOVE ALL SERIAL FOR NEUROHAB
  ...
  // Serial.print(".");
  // Serial.print((millis() - init_millis) % 1000);
  // Serial.print(" | ");
  // Serial.print(cur_millis);
  // Serial.println();
  ReadBatteryLevel();
  // UpdateDisplay();  // REMOVE FOR NEUROHAB
  goToSleep();
}
...
**************************************************************************************************************************************************/
// helper functions
// Returns the pin that the pellet well sensor is on.
int FED3::getWellPin() {
  return PELLET_WELL;
}

// Lets the user set the event externally for logging.
void FED3::setEvent(String event) {
  Event = event;
}
...
void FED3::logLeftPoke(){
  if (PelletAvailable == false){
    BNC(TTLus, leftPoke);  // TTL for left poke
    leftPokeTime = millis();
    LeftCount ++;
    leftInterval = 0.0;
...
void FED3::logRightPoke(){
  if (PelletAvailable == false){
    BNC(TTLus, rightPoke);  // TTL for right poke
...
/**************************************************************************************************************************************************
                                                                                                Feeding functions
**************************************************************************************************************************************************/
void FED3::Feed(char color) {
  Feed(true, color);
}

void FED3::Feed(bool pixelsoff, char color) {
    ...
    if (pelletDispensed == true) {
      ...
      while (digitalRead (PELLET_WELL) == LOW and retInterval < 60000) {  //After pellet is detected, hang here for up to 1 minute to detect when it is removed
        ...
        if (digitalRead(LEFT_POKE) == LOW) {             //If left poke is triggered
          BNC(TTLus, leftWPellet);  // TTL for left poke
          leftPokeTime = millis();
          LeftCount ++;
          leftInterval = 0.0;
          while (digitalRead (LEFT_POKE) == LOW) {}  //Hang here until poke is clear
        ...
        if (digitalRead(RIGHT_POKE) == LOW) {            //If right poke is triggered
          BNC(TTLus, rightWPellet);  // TTL for right poke
          rightPokeTime = millis();
          RightCount ++;
           rightInterval = 0.0;
          while (digitalRead (RIGHT_POKE) == LOW) {}  //Hang here until poke is clear
      ...
      BNC(TTLus, retrieved);  // TTL for retrieval
...
// // triggers the BPOD to dispense a drop of water from the left water port on the SAMBox
// void FED3::dispenseLeftWater() {
//   BNC(TTLus, leftWater);  // TTL for left water dispense
// }

// // triggers the BPOD to dispense a drop of water from the right water port on the SAMBox
// void FED3::dispenseRightWater() {
//   BNC(TTLus, rightWater);  // TTL for right water dispense
// }
...
void FED3::ConditionedStimulus() {
  ConditionedStimulus(200);
}

void FED3::ConditionedStimulus(int duration) {
  ConditionedStimulus(200, 4000);
}
...
void FED3::ConditionedStimulus(int duration, int freq, char color, bool playTone) {
  ...
  if (playTone) {
    BNC(TTLus, stimTone);  // TTL for tone
...
void FED3::ConditionedStimulus_LED_ON(char color) {
  int r=0;
  int g=0;
  int b=0;
  int w=0;
...
void FED3::ConditionedStimulus_LED_OFF() {
  pixelsOff();
}
...
void FED3::BNC(int DELAY_US, int loops) {
  // This allows for open-drain signalling.
  ...
  // Reset to input to allow for draining.
  pinMode(BNC_OUT, INPUT);

  delay(PULSE_SEPAR_DELAY_MS);  // wait 100 ms after each BNC pulse so that we don't interrupt
}
...
void FED3::writeHeader() {
  ...
  if (tempSensor == false){
    logfile.println("MM:DD:YYYY hh:mm:ss,time_elapsed_ms,Library_Version,Session_type,Device_Number,Battery_Voltage,Motor_Turns,FR,FR_Count_Right,FR_Count_Left,Event,Active_Poke,Left_Poke_Count,Right_Poke_Count,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time,RPR_Right,RPR_Count_Right,RPR_Left,RPR_Count_Left");
  }


  if (tempSensor == true){
    logfile.println("MM:DD:YYYY hh:mm:ss,time_elapsed_ms,Temp,Humidity,Library_Version,Session_type,Device_Number,Battery_Voltage,Motor_Turns,FR,FR_Count_Right,FR_Count_Left,Event,Active_Poke,Left_Poke_Count,Right_Poke_Count,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time,RPR_Right,RPR_Count_Right,RPR_Left,RPR_Count_Left");
  }
...
void FED3::logdata() {
  ...
  logfile.print(millis() - init_millis);
  ...
  logfile.print(FR_Count_Right);
  logfile.print(",");

  logfile.print(FR_Count_Left);
  logfile.print(",");
  ...
  if (Event != "Pellet"){
    ...
    logfile.print("Timed_out"); // print "Timed_out" if retrieval interval is >60s
  }
  else {
  ...
  if (Event == "Pellet"){
    ...
    logfile.print(leftInterval/1000.000); // print left poke timing
    logfile.print(",");
  }

  else if ((Event == "Right") or (Event == "RightShort") or (Event == "RightWithPellet") or (Event == "RightinTimeout") or (Event == "RightDuringDispense")) {  // 
    logfile.print(rightInterval/1000.000); // print left poke timing
    logfile.print(",");
  }
  ...
  // RPR RPR_Count.
  logfile.print(RPR_Right);
  logfile.print(",");

  logfile.print(RPR_Count_Right);
  logfile.print(",");

  logfile.print(RPR_Left);
  logfile.print(",");

  logfile.println(RPR_Count_Left);
...
void dateTime(uint16_t* date, uint16_t* time) {
  DateTime now = pointerToFED3->rtc.now();
...
void FED3::SelectMode() {
  ...
  if (ClassicFED3==true){
  if (FEDmode == 0) display.print("0: Free_Feeding");
  if (FEDmode == 1) display.print("1: DualPortTimed");
  ...
  if (FEDmode == 9) display.print("9: Medusa-48h");
  if (FEDmode == 10) display.print("10: Sparrow");
  if (FEDmode == 11) display.print("11:Reverse Bellagio 48");
    display.refresh();
  }
